			...
			<vue-better-scroll  ref="scroll"  class="wrapper"
				...
				<div v-if="f1">
					<div class="weui_cells_access">
						...
						<div class="top" v-if="person==2 && work==2 && contact==2">认证信息</div>
						<div v-if="person==2 && work==2 && contact==2">
					...
					<div class="button-wrap mgt-20">
		                ...
		                <mt-button v-if="person==2 && work==2 && contact==2 && af2 != 4" type="primary" size="large" :disabled="btn1State" @click="stages">分期申请</mt-button>
		                <p v-if="person==2 && work==2 && contact==2 && af2 != 4" class="font-size-12 mgt-16 pdl-10">点击分期申请代表确认并阅读<span class="blue-class" @click="jkfqxy">《借分期分期协议》</span></p>
		            </div>
				</div>
	            <br>
	            <br>
	...
	export default {
		...
		methods: {
			...
			commit1(){
				let _self = this;
				this.btn2State = true;
				api.antiFraud1({ tongdId:_fmOpt.getinfo(), userId: _self.userId}).then(res=>{
					var str = res.code+"";
					if(str=="100005"){
						// 判断首贷
						layer.open({
						    content: '通过系统初步审核，请继续完善其他信息'
						    ,style: 'width:74%;'
						    ,anim :'up'
						    ,btn: '确认',
						    yes : function(index){
						...
						//1:先判断用户是否被拒
					  	api.verUser({ accessToken: getCookie("thz_H5") }).then(res=>{
			               	if(res.body.userStat =="2"){
								layer.open({
								    content: '您的分期申请不通过，请在90天后再次尝试。'
								    ,style: 'width:74%;'
								    ,anim :'up'
								    ,btn: '确认',
								    yes : function(index){
								    	layer.close(index);
								    	_self.$router.replace({name:"generate"});
									}
							  	});
							}else{
								api.userInfoList({source:2}).then(res=>{
									let data = res.body;
									// 判断是否需要做反二
									if(data.bindCard==2 && data.face==2 && data.mobile==2 && data.wage==2){
										_self.$router.replace({ name : "progress" });	
									}else{
										_self.init();
									}
